...
export default function OnboardingCategoryPage() {
  ...
  const filteredPrompts = onboardingPrompts.filter(
    (prompt) =>
      (searchQuery === "" ||
        ...
        {filteredPrompts.length > 0 ? (
          <div className="space-y-6">
            {filteredPrompts.map((prompt) => (
              <Link key={prompt.id} href={`/categories/onboarding/${prompt.id}`} className="block">
                <div className="bg-white rounded-xl overflow-hidden shadow-md border-l-4 border-green-600 hover:shadow-lg transition-shadow">
                  ...
                  <div className="p-4 border-t border-gray-200">
                    <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 relative">
                      <div className="absolute top-2 right-2 text-green-500 px-2 py-0.5 rounded-md text-xs bg-green-50">
                        Technique RCT
                      </div>
                      ...
                      <div className="mt-4 flex justify-end">
                        <Button className="flex items-center space-x-2 text-green-600 hover:text-green-800 px-3 py-1.5 rounded-lg border border-green-200 bg-green-50 hover:bg-green-100 transition-colors">
