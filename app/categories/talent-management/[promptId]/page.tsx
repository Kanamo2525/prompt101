...
import { ArrowLeft, Clipboard, Star, Lightbulb, Check, BookOpen, Award } from "lucide-react"
...
export default function PromptDetailPage({ params }: { params: { promptId: string } }) {
  ...
  const prompt = talentManagementPrompts.find((p) => p.id === params.promptId)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Button asChild>
            <Link href="/categories/talent-management">Retour à la catégorie Gestion des talents</Link>
          </Button>
        </div>
      </div>
    )
  ...
  const relatedPrompts = talentManagementPrompts.filter((p) => p.id !== params.promptId).slice(0, 3)

  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-500 to-yellow-400">
        ...
        <div className="bg-white rounded-xl overflow-hidden shadow-xl">
          ...
          <div className="bg-amber-100 p-6 sm:p-8">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-amber-700 text-xl font-bold">Gestion des talents et mobilité interne</h2>
                  <div className="mt-1">
                    <span className="bg-amber-500 text-white px-3 py-1 rounded-full text-sm font-medium">#RH</span>
                  </div>
                </div>
              </div>
              <Button asChild variant="ghost" size="sm">
                <Link
                  href="/categories/talent-management"
            ...
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <div className="bg-amber-500 rounded-full w-16 h-16 flex items-center justify-center overflow-hidden">
          ...
          <div className="p-6 sm:p-8 border-t border-gray-200">
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-300 relative">
              <div className="absolute top-2 right-2 text-amber-500 px-2 py-1 rounded-md text-xs bg-amber-50">
                Technique RCT
              </div>
              ...
              <div className="mt-6 flex justify-end">
                <Button
                  ...
                  className="flex items-center space-x-2 text-amber-600 hover:text-amber-800 px-3 py-1.5 rounded-lg border border-amber-200 bg-amber-50 hover:bg-amber-100 transition-colors"
                >
                  {copied ? (
                    <>
              ...
              <div className="absolute right-0 bottom-0 transform translate-y-1/4 translate-x-1/4 opacity-20 pointer-events-none">
                <div className="bg-amber-200 h-24 w-24 rounded-full flex items-center justify-center">
                  <span className="text-amber-500 font-bold text-xl">Ctrl+C</span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-50 p-6 sm:p-8 border-t border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {relatedPrompts.map((relatedPrompt) => (
                <Link key={relatedPrompt.id} href={`/categories/talent-management/${relatedPrompt.id}`}>
