...
import { ArrowLeft, Clipboard, Star, Lightbulb, Check, BookOpen, BookMarked } from "lucide-react"
...
export default function PromptDetailPage({ params }: { params: { promptId: string } }) {
  ...
  const prompt = trainingPrompts.find((p) => p.id === params.promptId)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Button asChild>
            <Link href="/categories/training">Retour à la catégorie Formation</Link>
          </Button>
        </div>
      </div>
    )
  ...
  const relatedPrompts = trainingPrompts.filter((p) => p.id !== params.promptId).slice(0, 3)

  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-500 to-indigo-400">
      <div className="max-w-5xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="bg-white rounded-xl overflow-hidden shadow-xl">
          {/* Header */}
          <div className="bg-purple-100 p-6 sm:p-8">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-purple-700 text-xl font-bold">Formation et développement des compétences</h2>
                  <div className="mt-1">
                    <span className="bg-purple-500 text-white px-3 py-1 rounded-full text-sm font-medium">#RH</span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center">
              <div className="flex-shrink-0">
                <div className="bg-purple-500 rounded-full w-16 h-16 flex items-center justify-center overflow-hidden">
                  <BookMarked className="h-8 w-8 text-white" />
                </div>
              </div>
              <div className="ml-5">
                <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 flex items-center">
                  <span className="text-purple-500 mr-2">{prompt.id}.</span>
                  {prompt.title}
                </h1>
          ...
          <div className="p-6 sm:p-8 border-t border-gray-200">
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-300 relative">
              <div className="absolute top-2 right-2 text-purple-500 px-2 py-1 rounded-md text-xs bg-purple-50">
                Technique RCT
              </div>
              ...
              <div className="mt-6 flex justify-end">
                <Button
                  ...
                  className="flex items-center space-x-2 text-purple-600 hover:text-purple-800 px-3 py-1.5 rounded-lg border border-purple-200 bg-purple-50 hover:bg-purple-100 transition-colors"
                >
                  {copied ? (
                    <>
              ...
              <div className="absolute right-0 bottom-0 transform translate-y-1/4 translate-x-1/4 opacity-20 pointer-events-none">
                <div className="bg-purple-200 h-24 w-24 rounded-full flex items-center justify-center">
                  <span className="text-purple-500 font-bold text-xl">Ctrl+C</span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-gray-50 p-6 sm:p-8 border-t border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {relatedPrompts.map((relatedPrompt) => (
                <Link key={relatedPrompt.id} href={`/categories/training/${relatedPrompt.id}`}>
          ...
          {/* Bottom tip */}
          <div className="p-4 bg-purple-50 border-t border-purple-200 flex items-start">
            <div className="flex-shrink-0 text-purple-500 mr-3">
              <Lightbulb className="h-6 w-6" />
            </div>
            <p className="text-purple-800 text-sm">
              Pour plus de pertinence, adaptez ce prompt à votre contexte spécifique et aux objectifs pédagogiques de
              votre formation.
            </p>
          </div>
        </div>
